...
export default function Dashboard() {
  ...
  const stats = {
    ...
    overduePayments: payments.filter(r => r.status === 'overdue').length,
    monthlyCollection: payments.filter(r => r.status === 'paid' && r.paymentDate && new Date(r.paymentDate).getMonth() === new Date().getMonth()).reduce((sum, r) => sum + (r.paidAmount || 0), 0)
  };
