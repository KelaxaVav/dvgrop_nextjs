...
interface ApprovalDetailsProps {
  loan: ILoan;
  customer?: Customer;
  onApprove: (loan: ILoan, approvedAmount: number, remarks?: string) => void;
  onReject: (loan: ILoan, remarks: string) => void;
  onClose: () => void;
  canApprove: boolean;
}
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-blue-50 p-6 rounded-xl">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="text-blue-600 font-medium">Type:</span> {capitalizeFirstLetter(loan?.type)}</div>
                  ...
                  <div className="col-span-2"><span className="text-blue-600 font-medium">Purpose:</span> {capitalizeFirstLetter(loan?.purpose)}</div>
                </div>
              </div>
              ...
              {customer && (
                <div className="bg-green-50 p-6 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div><span className="text-green-600 font-medium">Name:</span> {capitalizeFirstLetter(customer?.name)}</div>
                    ...
                    <div><span className="text-green-600 font-medium">Occupation:</span> {capitalizeFirstLetter(customer?.occupation)}</div>
                    <div><span className="text-green-600 font-medium">Monthly Income:</span> LKR {customer?.income.toLocaleString()}</div>
                    <div><span className="text-green-600 font-medium">Marital Status:</span> {capitalizeFirstLetter(customer?.maritalStatus)}</div>
              ...
              {loan.guarantor && (
                <div className="bg-yellow-50 p-6 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="col-span-2"><span className="text-yellow-600 font-medium">Address:</span> {capitalizeFirstLetter(loan?.guarantor?.address)}</div>
                  </div>
                </div>
              )}
              ...
              {loan?.collateral && (
                <div className="bg-purple-50 p-6 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div><span className="text-purple-600 font-medium">Type:</span> {capitalizeFirstLetter(loan?.collateral?.type)}</div>
                    <div><span className="text-purple-600 font-medium">Value:</span> LKR {loan?.collateral?.value?.toLocaleString()}</div>
                    <div className="col-span-2"><span className="text-purple-600 font-medium">Description:</span> {capitalizeFirstLetter(loan?.collateral?.description)}</div>
                  ...
                  {loan?.documents?.map((doc) => (
                    <div key={doc?._id} className="flex items-center justify-between p-3 bg-white rounded-lg border">
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="font-medium text-gray-800">{capitalizeFirstLetter(doc?.name)}</p>
            ...
            <div className="space-y-6">
              <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                ...
                <div className="text-center mb-4">
                  <div className={`text-4xl font-bold mb-2 ${assessment?.score >= 70 ? 'text-green-600' :
                    assessment?.score >= 40 ? 'text-yellow-600' : 'text-red-600'
                    }`}>
                    {assessment?.score}/100
                  </div>
                  <div className={`px-3 py-1 rounded-full text-sm font-medium ${assessment?.recommendation === 'approve' ? 'bg-green-100 text-green-800' :
              ...
              {loan?.status !== 'pending' && (
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6">
                  ...
                  <div className={`p-4 rounded-lg ${loan?.status === 'approved' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
                    }`}>
                    ...
                    {loan?.approvedBy && (
                      <p className="text-sm">By: {loan?.approvedBy?.name ?? ""}</p>
                    )}
                    {loan?.approvedDate && (
